...
- ✅ Исправлено — product version `KMUX_VERSION=0.1.0` отделена от версии совместимости терминальной базы `KONSOLE_VERSION=26.07.70`. About data и плагины используют версию Kmux, а shell environment и terminal version report — версию upstream Konsole; regression test проверяет экспорт `KONSOLE_VERSION=260770`.
- ✅ Исправлено — сборки без DBus используют `QLocalServer` activation fallback с тем же args/cwd/environment payload, что и DBus. Общий startup lock сериализует discovery и публикацию server, поэтому только primary владеет workspace state; secondary возвращает результат `Application::requestActivation`. Round-trip test проверяет transport, а отдельная `USE_DBUS=OFF` сборка проверяет fallback branch.
