...
### Medium-1. Обычные MRU tab shortcuts переключают проекты
...
### Medium-2. Перенос фоновой вкладки меняет active tab исходного проекта
...
### Medium-3. Клик по notification фонового проекта не активирует этот проект
...
### Medium-4. Cold restore теряет флаги пользовательских tab overrides
...
### Medium-5. Profile `Directory` всегда перетирается activation cwd
...
### Medium-6. Относительные `--layout` и `--tabs-from-file` используют cwd старого процесса
